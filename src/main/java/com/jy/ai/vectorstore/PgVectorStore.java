...
import org.springframework.ai.vectorstore.PgVectorFilterExpressionConverter;
import org.springframework.ai.vectorstore.PgVectorSchemaValidator;
...
public class PgVectorStore extends AbstractObservationVectorStore implements InitializingBean {
    ...
    public static final String DEFAULT_TABLE_NAME = "langchain_pg_embedding";
    ...
    public PgVectorStore(JdbcTemplate jdbcTemplate, EmbeddingModel embeddingModel) {
        this(jdbcTemplate, embeddingModel, 1024, PgDistanceType.COSINE_DISTANCE, false,
            PgIndexType.HNSW, false);
    }
    ...
    public List<Document> doSimilaritySearch(SearchRequest request) {
        ...
        if (StringUtils.hasText(nativeFilterExpression)) {
            jsonPathFilter = " AND cmetadata::jsonb @@ '" + nativeFilterExpression + "'::jsonpath ";
        }
        ...
        String sqlTemplate = "SELECT *, embedding <=> ? AS distance FROM %s WHERE embedding <=> ? < ? %s ORDER BY distance LIMIT ? ";

        return this.jdbcTemplate.query(
            String.format(sqlTemplate, getFullyQualifiedTableName(), "2d1e717b-8a6e-486f-8793-f836d1f6bf1c", jsonPathFilter),
    ...
    private static class DocumentRowMapper implements RowMapper<Document> {
        ...
        private static final String COLUMN_METADATA = "cmetadata";

        private static final String COLUMN_ID = "uuid";

        private static final String COLUMN_CONTENT = "document";
